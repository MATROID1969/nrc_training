...
# ------------------------------------------------------------
# Fájlútvonal GitHub / Streamlit Cloud kompatibilisen
# ------------------------------------------------------------
BASE_DIR = Path(__file__).parent
DATA_PATH = BASE_DIR / "streamlit_base.xlsx"
...
with tab2:
    ...
    with filter_col2:
        with st.container(border=True):
            ...
            if len(category_columns) < 2:
                st.warning("Nincs legalább két megfelelő kategória változó.")
                category_1 = None
                category_2 = None
                diagram_type = None
            else:
    ...
    with chart_col2:
        with st.container(border=True):
            ...
            if category_1 is None or category_2 is None:
                st.info("Válassz ki két kategória változót.")

            elif category_1 not in df1.columns or category_2 not in df1.columns:
                st.error("A kiválasztott oszlop nem található az adatban.")
...
with tab3:
    ...
    with filter_col3:
        with st.container(border=True):
            ...
            if len(list_columns) == 0:
                st.warning("Nincs lista típusú változó.")
                selected_list_col = None
            else:
    ...
    with chart_col3:
        with st.container(border=True):
            ...
            if selected_list_col is None:
                st.info("Nincs megjeleníthető lista típusú változó.")
                ...
                if list_counts_df1.empty:
                    st.warning("Ehhez a változóhoz nem találtam listás válaszokat.")
